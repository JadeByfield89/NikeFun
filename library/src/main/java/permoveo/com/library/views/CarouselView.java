...
//    }
    ...
    private int getViewLeft(View view) {
        ...
        int halfViewWidth = view.getWidth() / 2;

        int viewLeft = getScreenCenterX() - halfViewWidth;

        return viewLeft;

    }

    private int CountAndAnimateVisibleChildren() {
        if (this == null) return 0;
        int count = 0;
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i).getVisibility() == View.VISIBLE) {
                count++;
                Log.d("CarouselView", "current count -> " + i);

                CircleImageView image = (CircleImageView) getChildAt(i).findViewById(R.id.iv_pager_item);
                int imagePosition = (int) image.getTag();
                Log.d("CarouselView", "Tag of visible view -> " + imagePosition);
                Log.d("CarouselView", "Left of visible view -> " + getChildAt(i).getLeft());

                if (mLastMotionX < getScreenCenterX()) {


                    float scaleBy = mLastMotionX / 350;

                    if (scaleBy > 1.0f) {
                        getChildAt(0).setScaleX(scaleBy);
                        getChildAt(0).setScaleY(scaleBy);

                        getChildAt(1).setScaleX(mLastMotionX / 300);
                        getChildAt(1).setScaleY(mLastMotionX / 300);

                        getChildAt(2).setScaleX(mLastMotionX / 250);
                        getChildAt(2).setScaleY(mLastMotionX / 250);

                    }

                    Log.d("CarouselView", "Scaling UP");

                } else {

                    scaleDownView(getChildAt(3));
                    scaleDownView(getChildAt(4));

                    Log.d("CarouselView", "Scaling DOWN");
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    Log.d("CarouselView", "Scroll delta X -> " + deltaX);
                    Log.d("CarouselView", "Scroll last X -> " + mLastMotionX);


                    scrollByDelta(deltaX);

                    Log.d("CarouselView", "Motion SCROLLING, selected view position -> " + getSelection());

                    // Check if the center of the selected view has passed the middle of the screen, if so
                    // Begin to animate it down back to it's original size
                    Log.d("CarouselView", "Screen center X -> " + getScreenCenterX());
                    Log.d("CarouselView", "Selected view center X -> " + getViewLeft(getSelectedView()));
                    Log.d("CarouselView", "Number of visible children -> " + CountAndAnimateVisibleChildren());
                ...
                Log.d("CarouselView", "Scroll delta X -> " + deltaX);
                Log.d("CarouselView", "Scroll last motion X -> " + mLastMotionX);
            ...
            case MotionEvent.ACTION_UP:
                ...
                clearChildrenCache();
                mTouchState = TOUCH_STATE_RESTING;
                scaleDownView(getSelectedView());
                Log.d("CarouselView", "Motion UP, selected view position -> " + getSelection());
